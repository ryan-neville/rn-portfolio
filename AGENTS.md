...
[projects/japanese-flashcard-app](projects/japanese-flashcard-app) and [projects/vowel-movement](projects/vowel-movement) are **git submodules**, each with its own repository and its own `AGENTS.md`. Their files are compiled by this project's toolchain, so a change there affects both, but edits belong to those repos and follow their rules — including their own browser-compatibility sections, which cover the flashcard 3D flip and the game's pointer-drag interaction. Do not commit changes to submodule files from this repository.

Both are mounted as routes under [app/projects/](app/projects/), importing the submodule's `app/page` but not its root layout. The wiring each one needs — Tailwind content globs, `@/` path mappings in `tsconfig.json`, whether its global stylesheet is imported — differs per submodule and is documented in the README. The vowel-movement route is the one that imports a submodule stylesheet; because that stylesheet restyles `body`, that route must link back to the site with a plain `<a>`, not `next/link`.

**A submodule's `package.json` is never installed — this repo's is.** Vercel runs `npm install` only at the repository root and checks submodules out as bare source, with no `node_modules` of their own. Node resolves a bare import by walking up from the importing file, so a submodule import like `@dnd-kit/core` finds `projects/japanese-flashcard-app/node_modules` on a machine where you have ever run `npm install` inside that submodule — the build then passes locally and fails on Vercel with `Module not found`. Every runtime dependency a submodule imports must be mirrored into this repo's [package.json](package.json); the `@dnd-kit/*` entries are there for the flashcard app's drag-to-reorder, not for the portfolio itself. **Whenever you bump a submodule pointer, diff that submodule's `dependencies` against this one's and install anything new here as well** — adding a route is not the only thing that can break a deploy, a submodule adding a package does it too.
